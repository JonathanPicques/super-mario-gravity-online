...
func _process(delta):
	...
	if peer != null and peer.id == peer_id:
		if Input.is_action_just_pressed("ui_left"):
			Game.rpc("net_peer_post_configure", peer.id, Game.Players.get_next_player_index(peer.player), false)
...
func set_peer(new_peer):
	...
	if peer != null:
		...
		$CharacterSprite.texture = load(Game.Players.get_player(peer.player).preview)
